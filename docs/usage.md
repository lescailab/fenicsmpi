# nf-core/fenicsmpi: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/fenicsmpi/usage](https://nf-co.re/fenicsmpi/usage)
...
```console
nextflow run nf-core/fenicsmpi --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/fenicsmpi
```
...
First, go to the [nf-core/fenicsmpi releases page](https://github.com/nf-core/fenicsmpi/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
